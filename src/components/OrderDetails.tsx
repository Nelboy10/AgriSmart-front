      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6">
        ...
        <div>
          ...
          <div className="space-y-4">
            <div>
              <h3 className="font-medium text-gray-700">Produit</h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1">{order.client.username}</p>
              </div>
              <div>
                <h3 className="font-medium text-gray-700">Agriculteur</h3>
                <p className="mt-1">{order.farmer.username}</p>
              </div>
            </div>
            
            <div>
              <h3 className="font-medium text-gray-700">Livraison prévue</h3>
              {order.delivery_schedule ? (
                <div className="mt-1">
                  {/* Afficher la date spécifique si elle existe */}
                  {order.delivery_schedule.delivery_date ? (
                    <p className="font-medium">
                      {formatDate(order.delivery_schedule.delivery_date)}
                    </p>
                  ) : (
                    <p>
                      {getDayOfWeekName(order.delivery_schedule.day_of_week)}: 
                      {order.delivery_schedule.start_time} - {order.delivery_schedule.end_time}
                    </p>
                  )}
                  <p className="text-sm text-gray-500">
                    {order.delivery_schedule.location}
                  </p>
                </div>
              ) : (
                <p className="mt-1">Non spécifiée</p>
              )}
            </div>
            
            {order.rejection_reason && (
              <div className="bg-red-50 border-l-4 border-red-500 p-4">
                <h3 className="font-medium text-red-700">Raison du rejet</h3>
                <p className="mt-1 text-red-600">{order.rejection_reason}</p>
              </div>
            )}
          </div>
          ...
          {(isFarmer || isClient) && (
            <div className="mt-8">
              ...
              {/* Actions pour agriculteur/admin */}
        ...
        <div>
              ...
              {order.status_history.map(update => (
                ...
                >
                  ...
                  <p className="text-sm mt-1">
                    Par: {update.changed_by.username}
                  </p>
                  {update.notes && (
          ...
          {order.delivery_confirmation && (
            <div className="mt-8">
              <h2 className="text-xl font-semibold mb-4">Confirmation de livraison</h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  <h3 className="font-medium">Agriculteur</h3>
                  <p className="mt-1">
